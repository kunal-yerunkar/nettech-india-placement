...
const AdminDashboard = () => {
  ...
  const archivesPagination = usePagination(archives, ITEMS_PER_PAGE);
  const reelsPagination = usePagination(reels, ITEMS_PER_PAGE);
  const partnersPagination = usePagination(partners, ITEMS_PER_PAGE);
  const domainsPagination = usePagination(domains, ITEMS_PER_PAGE);
  const registrationsPagination = usePagination(registrations, ITEMS_PER_PAGE);
  const partnerLeadsPagination = usePagination(partnerLeads, ITEMS_PER_PAGE);
  const inquiriesPagination = usePagination(inquiries, ITEMS_PER_PAGE);
  ...
  const loadAllData = async () => {
    ...
    archivesPagination.reset();
    reelsPagination.reset();
    partnersPagination.reset();
    domainsPagination.reset();
    registrationsPagination.reset();
    partnerLeadsPagination.reset();
    inquiriesPagination.reset();
  };
  ...
  const getPaginationObject = () => {
    const paginationMap = {
      'archives': archivesPagination, 'reels': reelsPagination, 'partners': partnersPagination,
      'student-leads': registrationsPagination, 'partner-leads': partnerLeadsPagination,
      'web-inquiries': inquiriesPagination, 'domain-hub': domainsPagination
    };
    return paginationMap[activeTab] || archivesPagination;
  };
  ...
  // Merge filtered data with pagination
  const paginatedData = useMemo(() => {
    ...
    const pagination = paginationMap[activeTab] || archivesPagination;
  ...
  }, [searchTerm, archives, reels, partners, domains, registrations, partnerLeads, inquiries, activeTab, archivesPagination, reelsPagination, partnersPagination, domainsPagination, registrationsPagination, partnerLeadsPagination, inquiriesPagination]);
        ...
        <div className="p-10">
          {activeTab === 'form-architect' ? (
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1">
                          ...
                          <select value={field.type} onChange={(e) => updateFormField(field.id, { type: e.target.value })} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-[10px] font-black focus:border-blue-600 outline-none appearance-none">
                            <option value="text" className="bg-[#0a0f18] text-white">TEXT</option>
                            <option value="email" className="bg-[#0a0f18] text-white">EMAIL</option>
                            <option value="tel" className="bg-[#0a0f18] text-white">PHONE</option>
                            <option value="url" className="bg-[#0a0f18] text-white">URL</option>
                            <option value="number" className="bg-[#0a0f18] text-white">NUMBER</option>
                          </select>
                        </div>
                        <div className="space-y-1">
            ...
            <div className="bg-[#0a0f18] rounded-[2.5rem] border border-white/5 overflow-hidden shadow-2xl">
              ...
              {/* Pagination Controls */}
              {!['form-architect'].includes(activeTab) && paginatedData.pagination.totalPages > 1 && (
                <PaginationControls
                  ...
                  onPrevPage={() => getPaginationObject().prevPage()}
                  onNextPage={() => getPaginationObject().nextPage()}
                  onGoToPage={(page) => getPaginationObject().goToPage(page)}
                />
              )}
            </div>
          )}
                    ...
                    <div className="md:col-span-2">
                      ...
                      {viewOnly ? (
                        ...
                        <select name="interestedDomain" defaultValue={editingItem?.interestedDomain} onChange={(e) => setTempDomain(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-xl px-5 py-4 text-[11px] font-bold focus:border-blue-600 outline-none text-white appearance-none">
                          <option value="" className="bg-[#0a0f18] text-white">SELECT DOMAIN</option>
                          {JOB_DOMAINS.map(d => <option key={d.id} value={d.title} className="bg-[#0a0f18] text-white">{d.title}</option>)}
                        </select>
                      )}
                    </div>
                  </div>
